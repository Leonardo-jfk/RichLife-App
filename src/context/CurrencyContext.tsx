...
interface CurrencyInfo {
  ...
  symbol: string;
  name: string;
  locale: string; // <-- On ajoute la locale pour chaque devise
}
...
const CURRENCIES: CurrencyInfo[] = [
  ...
  { code: "ARS", symbol: "$", name: "Peso Argentin", locale: "es-AR" },
];
...
interface CurrencyContextType {
  ...
  setCurrency: (currency: CurrencyType) => void;
  formatCurrency: (amount: number) => string;
  getCurrencyInfo: (code: CurrencyType) => CurrencyInfo;
}
...
export const CurrencyProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const loadCurrency = async () => {
    try {
      const savedCurrency = await AsyncStorage.getItem("@app_currency");
      const validCurrencies = CURRENCIES.map((c) => c.code);
      if (savedCurrency && validCurrencies.includes(savedCurrency as CurrencyType)) {
        setCurrencyState(savedCurrency as CurrencyType);
      }
    } catch (error) {
      console.error("Erreur chargement devise:", error);
  ...
  const formatCurrency = (amount: number) => {
    ...
    // Intl.NumberFormat gère automatiquement la position du symbole et les séparateurs selon la locale !
